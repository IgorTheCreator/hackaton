...
export class ProjectService {
  ...
  private calculateEnvironmentalScore(dto: CreateProjectDto): number {
    ...
    score += this.calculateScore(
      dto.environmental.wasteRecycled &&
        dto.environmental.mainImpact?.type !== 'WasteRecycled'
        ? dto.environmental.wasteRecycled
        : 0,
      10,
      5,
    ...
    score += this.calculateScore(
      dto.environmental.waterSaved &&
        dto.environmental.mainImpact?.type !== 'WaterCleaned'
        ? dto.environmental.waterSaved
        : 0,
      1000,
      5,
  ...
  private calculateSocialScore(dto: CreateProjectDto): number {
    ...
    score += this.calculateScore(
      dto.social.communityEngagement?.enabled
        ? dto.social.communityEngagement.count || 0
        : 0,
      50,
      5,
    );
  ...
  private extractEnvironmentalData(dto: CreateProjectDto) {
    ...
    return {
      co2Reduction: dto.environmental.co2Reduction || (mainImpact?.type === 'CO2Reduction' ? mainImpact.value : null),
      treesPlanted: dto.environmental.treesPlanted || (mainImpact?.type === 'TreesPlanted' ? Math.floor(mainImpact.value) : null),
      waterSaved: dto.environmental.waterSaved || (mainImpact?.type === 'WaterCleaned' ? mainImpact.value : null),
      wasteRecycled: dto.environmental.wasteRecycled || (mainImpact?.type === 'WasteRecycled' ? mainImpact.value : null),
  ...
  private extractSocialData(dto: CreateProjectDto) {
    return {
      ...
      communityEngagement: dto.social.communityEngagement?.enabled ? dto.social.communityEngagement.count : null,
      resourceAccess: dto.social.resourceAccess?.enabled ? dto.social.resourceAccess.description : null,
